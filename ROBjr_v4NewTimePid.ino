...
double goalHeading = 0;                 //Input, Output, and Setpoint of first PID
...
// Key Parameters
    ...
    const double goalTime = 0;  //Based off stepCount * travelDist = moving time and turnCount * turnTime
    const double turnTime = 0;  //Measure later with robot
    const double stepCount = 0;  //Set at event?
...
double goalSpeed, currentSpeed, speedError;
double Kp2=1.0, Ki2=1.0 , Kd2=0;

//Specify the links and initial tuning parameters
double blankZero = 0;
double myError = 0;
double historySpeed;                 //Distance Traveled / Time
PID myPID(&blankZero, &motorA_offset, &myError, Kp, Ki, Kd, DIRECT);
PID myPID2(&historySpeed, &currentSpeed, &goalSpeed, Kp2, Ki2, Kd2, DIRECT);
...
void setup() {
  ...
  // init encoders
    ...
    goalSpeed = stepCount * 50 / (goalTime - turnCount * turnTime);
}
...
void loop() {
  ...
  historySpeed = previousMillis / distTraveled;
...
void forwardWaterBottle() {
  movestraightF(1.0);
}

void backwardWaterBottle() {
  movestraightB(-1.0);
}
...
void KeepStraightF() {
  ...
  myPID.Compute();
  myPID2.Compute();
  motors.setSpeedA(goalSpeed);
  motors.setSpeedB(goalSpeed);
  motors.forward();
  printSomeInfo();
}
  ...
  if(millis() - previousMillis > print_time)
    ...
    // Print motor info in Serial Monitor
     ...
     // Start New Line
      ...
      Serial.print("Kp2 = ");
      Serial.print(myPID2.GetKp2());
      Serial.print(" Ki2 = ");
      Serial.print(myPID2.GetKi2());
      Serial.print(" Kd2 = ");
      Serial.print(myPID2.GetKd2());
      Serial.print(" Mode = ");
      Serial.println(myPID2.GetMode());
